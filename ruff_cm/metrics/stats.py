...
def mean_ci(arr, *, ci: float = 0.95, axis: int = 0) -> tuple[np.ndarray, np.ndarray]:
    values = np.asarray(arr, dtype=float)
    n = np.sum(~np.isnan(values), axis=axis)
    mean = np.nanmean(values, axis=axis)
    half_width = sem(values, axis=axis, nan_policy="omit") * t.ppf((1.0 + ci) / 2.0, np.maximum(n - 1, 1))
...
def smooth_curve_ci(df, *, value_col: str, group_col: str = "position", window: int = 5, ci: float = 1.96):
    ...
    sem_values = curve["sem"]
    curve["ci_lo"] = curve["mean"] - ci * sem_values
    curve["ci_hi"] = curve["mean"] + ci * sem_values
