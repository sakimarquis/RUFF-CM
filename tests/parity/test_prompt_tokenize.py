...
from ruff_cm.llm.spans import find_subsequences as legacy_find_subsequences
from ruff_cm.llm.spans import tokenize_with_loss_mask as legacy_tokenize_with_loss_mask
...
def test_find_subsequences_keeps_legacy_named_spans_and_supports_single_needles():
    assert find_subsequences(np.array([1, 2, 1, 2, 1]), {"pair": [1, 2]}) == {"pair": [(0, 2), (2, 4)]}
    assert legacy_find_subsequences([1, 2, 1, 2], {"pair": [1, 2]}) == {"pair": [(0, 2), (2, 4)]}
    assert find_subsequences([1, 2, 1, 2], [1, 2]) == [0, 2]


def test_tokenize_with_loss_mask_matches_legacy_spans_and_accepts_message_objects():
    ...
    legacy = legacy_tokenize_with_loss_mask(tokenizer, messages)
