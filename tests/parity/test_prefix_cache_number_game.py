...
def number_game_reference_serialize(cache):
    return {json.dumps(list(k), separators=(",", ":")): v for k, v in cache.items()}


def number_game_reference_load(raw):
...
def test_prefix_cache_codec_matches_number_game_fixture():
    ...
    assert serialize_prefix_cache(cache) == number_game_reference_serialize(cache)
    assert load_prefix_cache(serialize_prefix_cache(cache)) == number_game_reference_load(
        number_game_reference_serialize(cache)
    )
